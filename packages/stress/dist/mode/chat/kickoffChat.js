...
export async function kickoffChat(rootClient, cfg) {
    ...
    const { eventId } = await rootClient.sendMessage(announceChannelId, `hello, we're starting the stress test now!, containers: ${cfg.containerCount} ppc: ${cfg.processesPerContainer} clients: ${cfg.clientsCount} sessionId: ${sessionId}`);
    const initialStats = {
    ...
    await rootClient.sendMessage(announceChannelId, `System Info: ${makeCodeBlock(getSystemInfo())} Initial Stats: ${makeCodeBlock(initialStats)}`, { threadId: eventId });
    ...
    for (let i = 0; i < cfg.allWallets.length; i++) {
        ...
        const hasSpaceMembership = await rootClient.spaceDapp.hasSpaceMembership(spaceId, wallet.address);
        logger.log('minting membership for', i, wallet.address, 'has', hasSpaceMembership);
        if (!hasSpaceMembership) {
            const result = await rootClient.spaceDapp.joinSpace(spaceId, wallet.address, rootClient.connection.baseProvider.wallet);
...
async function startRootClient(client, balance, spaceId, defaultChannelId) {
...
//# sourceMappingURL=kickoffChat.js.map