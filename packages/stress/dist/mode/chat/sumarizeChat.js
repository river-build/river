...
export async function sumarizeChat(localClients, cfg) {
    ...
    const message = defaultChannel.view.timeline.find(channelMessagePostWhere((value) => value.body.includes(cfg.sessionId)));
    ...
    for (const client of localClients) {
        for (const channelId of cfg.channelIds) {
            ...
            const checkInMesssages = messages?.filter(channelMessagePostWhere((value) => value.body.includes(cfg.sessionId))) ?? [];
