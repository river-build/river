...
export async function joinChat(client, cfg) {
    ...
    await client.waitFor(() => client.spaceDapp.hasSpaceMembership(cfg.spaceId, client.connection.baseProvider.wallet.address), {
    ...
    if (client.clientIndex === cfg.localClients.startIndex) {
        ...
        await client.sendMessage(announceChannelId, `c${cfg.containerIndex}p${cfg.processIndex} Starting up! freeMemory: ${getSystemInfo().FreeMemory} clientStart:${cfg.localClients.startIndex} ${cfg.localClients.endIndex}`, { threadId: message.hashStr });
    }
    logger.log('emoji it');
    // emoji it
    ...
    for (const channelId of cfg.channelIds) {
        if (!client.streamsClient.streams
        ...
        await client.sendMessage(channelId, `${makeSillyMessage({ maxWords: 2 })}! ${cfg.sessionId}`);
    }
    logger.log('done');
}
// cruft we need to do for root user
async function startFollowerClient(client, spaceId, defaultChannelId) {
