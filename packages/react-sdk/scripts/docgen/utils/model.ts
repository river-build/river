...
export function createDataLookup(apiItem: model.ApiItem) {
    for (const child of apiItem.members) {
        createDataLookup(child)
    }
    ...
    if (
        ...
        // const module = (parent?.match(namespaceRegex) ?? parent?.match(moduleNameRegex))?.groups
        //     ?.module
...
function getLinkForApiItem(item: model.ApiItem) {
    ...
    if (item.kind === model.ApiItemKind.Namespace) {
        return baseLink
    }
...
//             lineNumber++
//             if (line.includes(' as ') || !line.includes(` ${functionName}`)) {
//                 continue
//             }
//             break
//         }
...
function lookahead(code: string, index_: number, left: AstNode): AstNode {
    ...
    if (node !== left) {
        return lookahead(code, node.end, node)
    }
    return node
}
...
function skipToPrimaryType(ast: AstNode): AstNode | null {
    ...
    if (ast.type === 'Union' || ast.type === 'Intersection') {
        if (ast.left.type === 'Keyword') {
            return skipToPrimaryType(ast.right)
        }
        if (ast.right.type === 'Keyword') {
            return skipToPrimaryType(ast.left)
        }
        return null
    }
    if (ast.type === 'TypeDeclaration') {
        return skipToPrimaryType(ast.expression)
    }
