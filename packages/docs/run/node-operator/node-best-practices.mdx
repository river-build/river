...
<Step title="Regular Updates">
    ...
    <br />
    <Note>If your node operator has not yet been fully registered or your node has not yet been registered, you can test running an unattached Node Fe to ensure health checks pass by setting `STANDBYONSTART=true`. A node started in this manner will not attach to Storage layer or shutdown if it is not registered yet on the River Chain.</Note>
</Step>
<Step title="Backups">
...
Using logs, metrics and profiling tools, you can identify and resolve issues with your node. Some issues may have specific resolutions on the [River Issue Tracker](https://github.com/river-build/river/issues?q=is%3Aissue+is%3Aclosed).

If there's a new bug or security vulnerability found, please file an issue on the [River Issue Tracker](https://github.com/river-build/river/issues/new/choose) for the core development team to address.
