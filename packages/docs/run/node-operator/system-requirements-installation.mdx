...
description: The following guide describes system requirements, dependencies, and installation details recommended for running a River Stream Node. This guide applies to nodes running in either Testnet or Mainnet.
---
...
<Accordion title="Node Frontend (FE) Requirements">
  ...
  Base Chain, River Chain, and cross-chain chain ids.
</Accordion>
...
Node FE's can be installed either from docker with the River public [ECR image registry](https://public.ecr.aws/h5v6m2x1/river) or built from [source code](https://github.com/river-build/river/tree/main/core/node).

#### Installation from Docker

<Accordion title="Install Image from River Public Image Registry">
The River Association maintains a public docker image registry `public.ecr.aws/h5v6m2x1/river` for images that are built officially from [source](https://github.com/river-build/river) during CI/CD.
...
River Stream Nodes run expecting certain environment variables to be present in the process environment and set at runtime. Config, which is described in [config.go](https://github.com/river-build/river/blob/main/core/config/config.go), is injected into the binary at runtime using [viper](https://github.com/spf13/viper).
