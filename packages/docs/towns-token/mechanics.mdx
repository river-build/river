...
- **Annual Inflation**: TOWNS's supply is inflated once per year through a publicly callable function, ensuring a predictable increase in token supply.
...
- **Bridging to Base**: TOWNS tokens are distributed on the Base blockchain. This involves either sending TOWNS tokens to a bridge protocol with the final destination being the distribution contract on Base or using a multisig wallet to bridge the tokens over to Base and then forward them to the distribution contract.
