...
This guide will help you get started with Towns Protocol React SDK, called River. You'll learn how to:
...
The easiest way to get started is using `create-river-build-app` and follow the instructions in the terminal. This will set up a new React project with all the necessary dependencies and configurations.
...
Towns has three networks:
- `omega` - The Towns mainnet, uses Base as the EVM chain
- `gamma` - The Towns testnet, uses Base Sepolia as the EVM chain

If you're not sure which network to use, `gamma` is a good starting point.

<Note>
You can use other gamma clients like [Towns (gamma)](https://app.gamma.towns.com) or the [River Playground](https://river-sample-app.vercel.app) to help you inspect messages on the `gamma` network.
</Note>
