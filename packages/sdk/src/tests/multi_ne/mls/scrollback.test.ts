...
describe('mlsScrollback', () => {
    test('clientRefreshesStreamOnBadSyncCookie', { timeout: 30_000 }, async () => {
        ...
        const alice = await makeTestClient({ nickname: 'alice' })
        ...
        await bob.setStreamEncryptionAlgorithm(streamId, MLS_ALGORITHM)
        await expect
            .poll(() => bob.mlsExtensions.agent?.streams?.get(streamId)?.localView?.status, {
                timeout: 10_000,
            })
            .toBe('active')
        await bob.stopSync()
