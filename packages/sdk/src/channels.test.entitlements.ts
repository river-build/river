...
describe('channelsWithEntitlements', () => {
    it('User who satisfies only one role ruledata requirement can join channel', async () => {
        const {
            alice,
            bob,
            alicesWallet,
    ...
    it('READ + REACT user can react and redact reactions, but cannot write (top-level or reply)', async () => {
        const { alice, bob, aliceSpaceDapp, spaceId, channelId } = await setupChannelWithCustomRole(
            ['alice'],
            NoopRuleData,
    ...
    it('WRITE user can write (top-level plus reply), react', async () => {
        const { alice, bob, aliceSpaceDapp, spaceId, channelId } = await setupChannelWithCustomRole(
            ['alice'],
            NoopRuleData,
    ...
    it('REACT + WRITE user can do all WRITE user can do', async () => {
        const { alice, bob, aliceSpaceDapp, spaceId, channelId } = await setupChannelWithCustomRole(
            ['alice'],
            NoopRuleData,
    ...
    it('banned user not entitled to channel', async () => {
        const {
            alice,
            alicesWallet,
            aliceSpaceDapp,
    ...
    it('user with only one entitlement from 3-nested NFT rule data can join channel', async () => {
    ...
    it('user booted on key request after entitlement loss', async () => {
        const testNftAddress = await TestERC721.getContractAddress('TestNFT')
        const { alice, alicesWallet, aliceSpaceDapp, bob, spaceId, channelId } =
            await setupChannelWithCustomRole([], getNftRuleData(testNftAddress))
    ...
    it('user cannot post after entitlement loss', async () => {
        const testNftAddress = await TestERC721.getContractAddress('TestNFT')
        const { alice, alicesWallet, aliceSpaceDapp, bob, spaceId, channelId } =
            await setupChannelWithCustomRole([], getNftRuleData(testNftAddress), [
                Permission.Read,
    ...
    it('ERC20 gate join pass - join as root, asset in linked wallet', async () => {
        const ruleData = treeToRuleData(await erc20CheckOp('TestERC20', 50n))
        const {
            alice,
            bob,
    ...
    it('ERC20 Gate Join Pass - join as linked wallet, assets in root wallet', async () => {
        const ruleData = treeToRuleData(await erc20CheckOp('TestERC20', 50n))
        const {
            alice,
            bob,
    ...
    it('ERC20 Gate Join Pass - assets split across wallets', async () => {
        const ruleData = treeToRuleData(await erc20CheckOp('TestERC20', 50n))
        const {
            alice,
            bob,
    ...
    it('eth balance gate pass', async () => {
    ...
    it('eth balance gate pass - across networks', async () => {
        const ruleData = treeToRuleData(ethBalanceCheckOp(oneEth))
    ...
    it('eth balance gate fail', async () => {
        const ruleData = treeToRuleData(ethBalanceCheckOp(oneEth))
    ...
    it('eth balance gate join pass - assets across wallets and networks', async () => {
        const ruleData = treeToRuleData(ethBalanceCheckOp(twoEth))
        const {
            alice,
            bob,
    ...
    it('adminsCanRedactChannelMessages', async () => {
