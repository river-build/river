...
class TestDriver {
    ...
    allExpectedReceived;
    badMessageReceived;
    constructor(client, num, testName) {
        this.client = client;
        this.num = num;
    ...
    eventDecrypted(streamId, contentKind, event) {
        ...
        this.log(`eventDecrypted channelId=${streamId} message=${content}`, this.expected ? [...this.expected] : undefined);
        if (this.expected?.delete(content)) {
            ...
            this.log(`channelNewMessage badMessageReceived text=${content}}, expected=${Array.from(this.expected?.values() ?? []).join(', ')}`);
            this.badMessageReceived(`badMessageReceived text=${content}, expected=${Array.from(this.expected?.values() ?? []).join(', ')}`);
        }
    }
    async step(channelId, stepNum, expected, message) {
...
const makeTestDriver = async (num, testName) => {
    const client = await makeTestClient();
    return new TestDriver(client, num, testName);
};
export const converse = async (conversation, testName) => {
    ...
    try {
        ...
        const drivers = await Promise.all(Array.from({ length: numDrivers })
            .fill('')
        ...
        const allJoinedSpace = Promise.all(others.map(async (d) => {
        ...
        log(`${testName} inviting others to space after`, others.map((d) => ({ num: d.num, userStreamId: d.client.userStreamId })));
        ...
        log(`${testName} inviting others to channel`, others.map((d) => ({ num: d.num, userStreamId: d.client.userStreamId })));
        const allJoined = Promise.all(others.map(async (d) => {
        ...
        for (const [conv_idx, conv] of conversation.entries()) {
            ...
            await Promise.all(conv.map(async (msg, msg_idx) => {
                ...
                const expected = new Set([...conv.slice(0, msg_idx), ...conv.slice(msg_idx + 1)].filter((s) => s !== ''));
                log(`conversation step execute ${msg_idx}`, msg, [...expected]);
                await drivers[msg_idx].step(channelId, conv_idx, expected, msg);
                log(`${testName} conversation step after send conv: ${conv_idx} msg: ${msg_idx}`, msg);
            }));
...
//# sourceMappingURL=testDriver.test_util.js.map