...
import { makeDonePromise, makeRandomUserAddress, makeTestClient, makeUniqueSpaceStreamId, waitFor, } from './util.test';
...
describe('userMetadataTests', () => {
    ...
    test('clientCanSetDisplayNamesInSpace', async () => {
        ...
        for (const client of [bobsClient, alicesClient]) {
            ...
            expect(streamView.getUserMetadata().displayNames.plaintextDisplayNames).toEqual(expected);
        }
    });
    test('clientCanSetDisplayNamesInDM', async () => {
        ...
        await expect(alicesClient.joinStream(streamId)).toResolve();
        await waitFor(() => {
            expect(stream.view.getMembers().membership.joinedUsers).toEqual(new Set([bobsClient.userId, alicesClient.userId]));
        });
        ...
        for (const client of [bobsClient, alicesClient]) {
            ...
            const clientDisplayNames = streamView.getUserMetadata().displayNames.plaintextDisplayNames;
    ...
    test('clientsPickUpDisplayNamesAfterJoin', async () => {
        ...
        await alicePromise.expectToSucceed();
        const expected = new Map([[bobsClient.userId, 'bob']]);
        const alicesClientDisplayNames = alicesClient.streams.get(streamId)?.view.membershipContent.userMetadata.displayNames
    ...
    test('clientCanSetUsernamesInSpaces', async () => {
        ...
        expect(bobsClient.streams.get(streamId).view.getUserMetadata().usernames.plaintextUsernames).toEqual(expected);
        ...
        for (const client of [bobsClient, alicesClient]) {
            ...
            expect(streamView.getUserMetadata().usernames.info(bobsClient.userId).username).toEqual('bob-username');
        }
    });
    ...
    test('clientCanSetEnsAddressesInSpace', async () => {
        ...
        for (const client of [bobsClient, alicesClient]) {
            const streamView = client.streams.get(streamId).view;
            expect(streamView.getUserMetadata().ensAddresses.confirmedEnsAddresses).toEqual(expected);
        }
    });
...
//# sourceMappingURL=userMetadata.test.js.map