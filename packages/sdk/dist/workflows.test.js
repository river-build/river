...
import { lastEventFiltered, makeRandomUserContext, makeTestRpcClient, makeUniqueSpaceStreamId, } from './util.test';
import { makeUniqueChannelStreamId, makeUserStreamId, streamIdToBytes, userIdFromAddress, } from './id';
import { getChannelUpdatePayload, getUserPayload_Membership, make_ChannelPayload_Inception, make_MemberPayload_Membership2, make_SpacePayload_Inception, make_UserPayload_Inception, } from './types';
...
describe('workflows', () => {
    ...
    test('creationSideEffects', async () => {
        ...
        let joinPayload = lastEventFiltered(await unpackStreamEnvelopes(userResponse.stream), getUserPayload_Membership);
        ...
        expect(userResponse.stream).toBeDefined();
        joinPayload = lastEventFiltered(await unpackStreamEnvelopes(userResponse.stream), getUserPayload_Membership);
        expect(joinPayload).toBeDefined();
        expect(joinPayload?.op).toEqual(MembershipOp.SO_JOIN);
