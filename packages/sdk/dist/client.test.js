...
import { DecryptionStatus } from '@river-build/encryption';
import { makeUserStreamId, makeUserSettingsStreamId, makeUserDeviceKeyStreamId, makeUserInboxStreamId, makeUniqueChannelStreamId, addressFromUserId, } from './id';
import { makeDonePromise, makeTestClient, makeUniqueSpaceStreamId, waitFor, getChannelMessagePayload, makeRandomUserAddress, } from './util.test';
import { CancelSyncResponse, ChannelMessage, SyncOp, SyncStreamsResponse, } from '@river-build/proto';
...
const createMockSyncGenerator = (shouldFail, updateEmitted) => {
    ...
    const generatorFunction = () => {
        ...
        if (syncCanceled) {
            log('emitting close');
            return Promise.resolve(new SyncStreamsResponse({
                syncId: 'mockSyncId',
...
describe('clientTest', () => {
    ...
    test('bobTalksToHimself-noflush', async () => {
        ...
        await expect(bobsClient.createChannel(bobsSpaceId, bobsChannelName, bobsChannelTopic, channelId)).toResolve();
    ...
    test('bobSendsBadPrevMiniblockHashShouldResolve', async () => {
        ...
        const encrypted = await bobsClient.encryptGroupEvent(new ChannelMessage({
            payload: {
                case: 'post',
                value: {
                    content: {
        ...
        await expect(bobsClient.makeEventWithHashAndAddToStream(channelId, message, Uint8Array.from(Array(32).fill(0)))).toResolve();
    });
    ...
    test('clientCreatesStreamsForExistingUser', async () => {
        ...
        expect(bobsAnotherClient.streams.get(makeUserDeviceKeyStreamId(bobsClient.userId))).toBeDefined();
    });
    test('bobCanSendMemberPayload', async () => {
        ...
        await expect(bobsClient.makeEventAndAddToStream(bobsClient.userSettingsStreamId, payload)).toResolve();
        ...
        await expect(bobsClient.makeEventAndAddToStream(bobsClient.userSettingsStreamId, payload)).rejects.toThrow('DUPLICATE_EVENT');
    });
    test('bobCreatesUnamedSpaceAndStream', async () => {
        ...
        await expect(bobsClient.createChannel(spaceId, channelName, channelTopic, channelId)).toResolve();
    ...
    test('bobSendsSingleMessage', async () => {
        ...
        const bobsChannelName = 'Bobs channel';
        const bobsChannelTopic = 'Bobs channel topic';
        await expect(bobsClient.createChannel(bobsSpaceId, bobsChannelName, bobsChannelTopic, bobsChannelId)).toResolve();
    ...
    test('bobDownloadsAlicesAndOwnDeviceKeys', async () => {
        ...
        bobsClient.on('userDeviceKeyMessage', (streamId, userId, deviceKeys) => {
            ...
            bobSelfInbox.runAndDone(() => {
                expect([bobUserDeviceKeyStreamId, aliceUserDeviceKeyStreamId]).toContain(streamId);
    ...
    test('bobDownloadsAlicesFallbackKeys', async () => {
        ...
        expect(fallbackKeys[alicesUserId].map((k) => k.fallbackKey)).toContain(Object.values(alicesClient.encryptionDevice.fallbackKey)[0]);
    });
    ...
    test('clientReturnsKnownDevicesForUserId', async () => {
        ...
        expect(knownDevices[0].fallbackKey).toBe(Object.values(alicesClient.encryptionDevice.fallbackKey)[0]);
    });
