...
describe('channelsTests', () => {
    ...
    test('clientsCanSendRedactionEvents', async () => {
        ...
        let eventId;
        await waitFor(() => {
            const event = channelStream.view.timeline.find((e) => getChannelMessagePayload(e.localEvent?.channelMessage) === 'Very bad message!');
        ...
        await waitFor(() => {
            const event = channelStream.view.timeline.find((e) => e.remoteEvent?.event.payload.case === 'channelPayload' &&
