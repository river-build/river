...
import { lastEventFiltered, makeEvent_test, makeTestRpcClient, makeUniqueSpaceStreamId, sendFlush, TEST_ENCRYPTED_MESSAGE_PROPS, waitForSyncStreams, waitForSyncStreamsMessage, } from './util.test';
import { makeUniqueChannelStreamId, makeUserStreamId, streamIdToBytes, userIdFromAddress, } from './id';
import { getChannelUpdatePayload, make_ChannelPayload_Inception, make_ChannelPayload_Message, make_MemberPayload_Membership2, make_SpacePayload_Inception, make_UserPayload_Inception, } from './types';
export const bobTalksToHimself = async (log, bobsContext, flush, presync) => {
    log('start');
    ...
    await maybeFlush();
    let presyncEvent = undefined;
    if (presync) {
        log('adding event before sync, so it should be the first event in the sync stream');
        presyncEvent = await makeEvent(bobsContext, make_ChannelPayload_Message({
    ...
    const bobSyncStreamIterable = bob.syncStreams({
        syncPos: [syncCookie],
    });
    await expect(waitForSyncStreams(bobSyncStreamIterable, async (res) => res.syncOp === SyncOp.SYNC_NEW && res.syncId !== undefined)).toResolve();
    if (flush || presync) {
        ...
        if (flush) {
            expect(stream.events).toEqual(presync ? [...channelEvents, presyncEvent] : channelEvents);
        }
        else {
    ...
    await maybeFlush();
    const badEvent = await makeEvent_test(bobsContext, make_ChannelPayload_Message({
        ...TEST_ENCRYPTED_MESSAGE_PROPS,
        ciphertext: 'hello',
...
//# sourceMappingURL=bob.test_util.js.map