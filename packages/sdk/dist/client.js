...
export class Client extends EventEmitter {
    ...
    async initializeUser(newUserMetadata) {
    ...
    async initUserDeviceKeyStream(metadata) {
    ...
    async createUserDeviceKeyStream(userDeviceKeyStreamId, metadata) {
    ...
    async makeEventAndAddToStream(streamId, payload, options = {}) {
        ...
        const { eventId, error } = await this.makeEventWithHashAndAddToStream(streamId, payload, prevHash, options.optional, options.localId, options.cleartext);
        return { eventId, error };
    }
    async makeEventWithHashAndAddToStream(streamId, payload, prevMiniblockHash, optional, localId, cleartext, retryCount) {
        ...
        try {
            ...
            if (errorContains(err, Err.BAD_PREV_MINIBLOCK_HASH) && retryCount < 3) {
                ...
                return await this.makeEventWithHashAndAddToStream(streamId, payload, bin_fromHexString(expectedHash), optional, isDefined(localId) ? eventId : undefined, cleartext, retryCount + 1);
            }
            else {
                if (localId) {
                    const stream = this.streams.get(streamId);
