...
contract ComputeLlamaCoreAddress is LlamaLensTestSetup {
  function test_ProperlyComputesAddress() public {
    address expected = address(
      lens.computeLlamaCoreAddress("Root Llama", LLAMA_INSTANCE_DEPLOYER)
    );
    assertEq(expected, address(rootCore));

    expected = address(
      lens.computeLlamaCoreAddress(
        "Mock Protocol Llama",
...
contract ComputeLlamaPolicyAddress is LlamaLensTestSetup {
  function test_ProperlyComputesAddress() public {
    address expected = address(
      lens.computeLlamaPolicyAddress("Root Llama", LLAMA_INSTANCE_DEPLOYER)
    );
    assertEq(expected, address(rootPolicy));

    expected = address(
      lens.computeLlamaPolicyAddress(
        "Mock Protocol Llama",
        LLAMA_INSTANCE_DEPLOYER
      )
    );
...
contract ComputeLlamaAccountAddress is LlamaLensTestSetup {
  function test_ProperlyComputesAddress() public {
    ...
    address expected = address(
      lens.computeLlamaAccountAddress(
...
contract ComputePermissionId is LlamaLensTestSetup {
  function test_ProperlyComputesId() public {
    PermissionData memory _pausePermission = PermissionData(
      address(mpPolicy),
      mpPolicy.setRolePermission.selector,
      mpStrategy1
    );
    bytes32 computedPausePermissionId = lens.computePermissionId(
      _pausePermission
    );
    assertEq(
      keccak256(
...
contract ComputeLlamaExecutorAddress is LlamaLensTestSetup {
  function test_ProperlyComputesAddress() public {
    address expected = address(
      lens.computeLlamaExecutorAddress(address(rootCore))
    );
    assertEq(expected, address(rootExecutor));

    expected = address(
      lens.computeLlamaExecutorAddress("Root Llama", LLAMA_INSTANCE_DEPLOYER)
    );
    assertEq(expected, address(rootExecutor));
    ...
    expected = address(
      lens.computeLlamaExecutorAddress(
        "Mock Protocol Llama",
        LLAMA_INSTANCE_DEPLOYER
      )
    );
