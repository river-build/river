...
library PolicyholderCheckpoints {
  ...
  function getAtProbablyRecentTimestamp(
    ...
    require(
      ...
      "PolicyholderCheckpoints: timestamp is not in the past"
    );
  ...
  function push(
    ...
    return
      _insert(
        ...
        LlamaUtils.toUint64(expiration),
        LlamaUtils.toUint96(quantity)
      );
  }
  ...
  function latestCheckpoint(
    ...
    internal
    view
    returns (bool exists, uint64 timestamp, uint64 expiration, uint96 quantity)
  {
    uint256 pos = self._checkpoints.length;
    if (pos == 0) {
      return (false, 0, 0, 0);
  ...
  function _insert(
    ...
    if (pos > 0) {
      ...
      require(
        ...
        "Role Checkpoint: invalid timestamp"
      );
