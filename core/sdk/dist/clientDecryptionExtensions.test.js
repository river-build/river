...
describe('ClientDecryptionExtensions', () => {
    let clients = [];
    const makeAndStartClient = async (opts) => {
    ...
    const getDecryptedChannelMessages = (stream) => {
        return stream.view.timeline
            ...
            // and local content
            if (e.localEvent?.channelMessage?.payload?.case === 'post' &&
    ...
    const waitForMessages = async (client, streamId, bodys) => {
    ...
    const getDecryptionErrors = async (client, streamId) => {
    ...
    const waitForDecryptionErrors = async (client, streamId, count) => {
