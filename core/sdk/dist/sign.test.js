...
import { checkDelegateSig, makeSignerContext } from './signerContext';
...
describe('sign', () => {
    ...
    const hash = bin_fromHexString('0x8dc27dbd6fc775e3a05c509c6eb1c63c4ab5bc6e7010bf9a9a80a42ae1ea56b0');
    const badHashes = [
    ...
    test('delegate-sig', async () => {
        ...
        const delegateA_context = await makeSignerContext(userWallet, delegateA_wallet, expiryA_none);
        const delegateB_context = await makeSignerContext(userWallet, delegateB_wallet, expiryB_valid);
    ...
    const makeContext = async (userPrivateKey, delegatePrivateKey) => {
    ...
    test.each(testParams)('sign-and-verify-%s', async (method, c, c2) => {
    ...
    test.each(testParams)('validate-prev-events-%s', async (method, c) => {
        const userStreamId = makeUserStreamId('0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
        const context = await c();
        expect(await makeEvent(context, make_UserPayload_Inception({
