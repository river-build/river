...
import { Envelope, EventRef, StreamEvent, Err, StreamAndCookie, SyncCookie, } from '@river-build/proto';
...
import { checkDelegateSig } from './signerContext';
import { keccak256 } from 'ethereum-cryptography/keccak';
export const _impl_makeEvent_impl_ = async (context, payload, prevMiniblockHash) => {
...
export const makeEvent = async (context, payload, prevMiniblockHash) => {
    ...
    if (prevMiniblockHash) {
        check(prevMiniblockHash.length === 32, `prevMiniblockHash should be 32 bytes, got ${prevMiniblockHash.length}`, Err.BAD_HASH_FORMAT);
    }
    return _impl_makeEvent_impl_(context, pl, prevMiniblockHash);
};
export const makeEvents = async (context, payloads, prevMiniblockHash) => {
    const events = [];
...
export const unpackStream = async (stream) => {
    assert(stream !== undefined, 'bad stream');
    const streamAndCookie = await unpackStreamAndCookie(stream);
    assert(stream.miniblocks.length > 0, `bad stream: no blocks ${streamIdAsString(streamAndCookie.nextSyncCookie.streamId)}`);
    ...
    assert(snapshot !== undefined, `bad block: snapshot is undefined ${streamIdAsString(streamAndCookie.nextSyncCookie.streamId)}`);
    const eventIds = [
        ...streamAndCookie.miniblocks.flatMap((mb) => mb.events.map((e) => e.hashStr), streamAndCookie.events.map((e) => e.hashStr)),
    ];
    return {
        streamAndCookie,
        snapshot,
...
export const unpackStreamEx = async (miniblocks) => {
    const streamAndCookie = new StreamAndCookie();
...
export const unpackStreamAndCookie = async (streamAndCookie) => {
    assert(streamAndCookie.nextSyncCookie !== undefined, 'bad stream: no cookie');
    const miniblocks = await Promise.all(streamAndCookie.miniblocks.map(async (mb) => await unpackMiniblock(mb)));
    return {
...
export const unpackMiniblock = async (miniblock, opts) => {
    check(isDefined(miniblock.header), 'Miniblock header is not set');
    const header = await unpackEnvelope(miniblock.header, opts);
    check(header.event.payload.case === 'miniblockHeader', `bad miniblock header: wrong case received: ${header.event.payload.case}`);
...
export const unpackEnvelope = async (envelope, opts) => {
    ...
    if (runChecks) {
        ...
        if (!hasElements(event.delegateSig)) {
            ...
            check(bin_equal(address, event.creatorAddress), 'Event signature is not valid', Err.BAD_EVENT_SIGNATURE);
        }
        else {
            checkDelegateSig({
...
export const unpackEnvelopes = async (event, opts) => {
    const ret = [];
...
export const unpackStreamEnvelopes = async (stream) => {
    const ret = [];
...
export const makeEventRef = (streamId, event) => {
    return new EventRef({
...
function numberToUint8Array64LE(num) {
...
function bigintToUint8Array64LE(num) {
    const buffer = new ArrayBuffer(8);
    const view = new DataView(buffer);
    view.setBigInt64(0, num, true); // true for little endian
    return new Uint8Array(buffer);
}
function pushByteToUint8Array(arr, byte) {
...
export function riverHash(data) {
    assertBytes(data);
...
export async function riverSign(hash, privateKey) {
    checkHash(hash);
...
export function riverVerifySignature(hash, signature, publicKey) {
    checkHash(hash);
    checkSignature(signature);
    return verify(signature.slice(0, 64), hash, publicKey);
}
export function riverRecoverPubKey(hash, signature) {
    checkHash(hash);
    checkSignature(signature);
...
export function publicKeyToUint8Array(publicKey) {
    ...
    check(typeof publicKey === 'string' && publicKey.startsWith('0x04') && publicKey.length === 132, 'Bad public key', Err.BAD_PUBLIC_KEY);
