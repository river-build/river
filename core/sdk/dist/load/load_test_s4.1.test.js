...
import { createAndStartClients, createClientSpaceAndChannel, extractComponents, getCurrentTime, getRandomSubset, getUserStreamKey, multipleClientsJoinSpaceAndChannel, sendMessageAfterRandomDelay, wait, } from './load.test_util';
...
describe('loadTestsScenario: Test application stability during long period of time under higher load. This scenario is supposed to be running for 3 hours and midnight PST for the first phase.', () => {
    test('create three users, space, channel, sent DM and GDM messages at random time in a minute window for 3 hours.', async () => {
        ...
        const clientWalletInfos = await createAndStartClients(accounts, jsonRpcProviderUrl, nodeRpcURL);
        const allClients = [];
        ...
        await multipleClientsJoinSpaceAndChannel(clientWalletInfos, spaceId, bobClientSpaceChannel.channelId);
                    ...
                    if (clearEvent.content.payload?.case === 'post' &&
                        ...
                        if (messageSet) {
                            if (messageSet.has(body)) {
                                ...
                                log(`client<${userStreamKey}> messageSet delete content successfully:`, body);
                            }
                            else {
                                log(`client<${userStreamKey}> messageSet does not have the content`, messageSet);
                            }
                        }
                        else {
        ...
        for (let interval = 0; interval < totalDuration; interval += windowDuration) {
            ...
            for (let a = 0; a < actionPerMinute; a += 1) {
                setTimeout(() => void startMessageSendingGDMWindow(windowIndex, allClients, futureMessagesMap, windowDuration), interval);
                log('Schedule sending DM message', { interval: interval, window: windowIndex });
                setTimeout(() => void startMessageSendingDMWindow(windowIndex, allClients, futureMessagesMap, windowDuration), interval);
            }
        }
        ...
        await waitFor(() => {
            ...
            for (const [key, messageSet] of futureMessagesMap) {
                if (messageSet.size > 0) {
                    log(`Non-empty set found for key ${key}, with ${messageSet.size} messages.`, messageSet);
    ...
    const startMessageSendingDMWindow = async (windowIndex, allClients, messagesSentPerUserMap, windownDuration) => {
        ...
        if (twoClients) {
            ...
            sendMessageAfterRandomDelay('dmChannelContent', senderClient, [receiptClient.userId], dmChannelReturnVal.streamId, windowIndex.toString(), messagesSentPerUserMap, windownDuration);
        }
    };
    const startMessageSendingGDMWindow = async (windowIndex, allClients, messagesSentPerUserMap, windownDuration) => {
        // Pick random clients
        ...
        for (let i = 0; i < gdmClients.length; i++) {
            ...
            if (creatorClient.userId !== client.userId &&
        ...
        sendMessageAfterRandomDelay('gdmChannelContent', senderClient, recipients, streamId, windowIndex.toString(), messagesSentPerUserMap, windownDuration);
    };
});
//# sourceMappingURL=load_test_s4.1.test.js.map