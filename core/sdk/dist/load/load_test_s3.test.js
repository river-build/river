...
import { createAndStartClients, createClientSpaceAndChannel, extractComponents, getCurrentTime, getUserStreamKey, multipleClientsJoinSpaceAndChannel, startMessageSendingWindow, wait, } from './load.test_util';
...
describe('loadTestsScenario3', () => {
    test('create three users, space, channel, sent messages to the channel at random time in a 10 minutes window for 3 hours.', async () => {
        ...
        for (let interval = 0; interval < totalDuration; interval += windowDuration) {
            ...
            setTimeout(() => startMessageSendingWindow('channelContent', windowIndex, allClients, channelId, futureMessagesMap, windowDuration), interval);
            ...
            messagesSentPerUserMap.forEach((sentMessagesSet, userStreamKey) => {
                if (sentMessagesSet.size === 0) {
                    ...
                    sentMessagesSet.forEach((message) => {
                        missingMessages.push(`Verification for ${userStreamKey} in window<${windowIndex}>, missing message: ${message}`);
                    });
                }
            });
        }
        ...
        await waitFor(() => {
            ...
            for (const [key, messageSet] of futureMessagesMap) {
                if (messageSet.size > 0) {
                    log(`Non-empty set found for key ${key}, with ${messageSet.size} messages, which means those messages are not received.`, messageSet);
...
//# sourceMappingURL=load_test_s3.test.js.map