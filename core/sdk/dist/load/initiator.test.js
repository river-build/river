...
import { createAndStartClient, createAndStartClients, multipleClientsJoinSpaceAndChannel, } from './load.test_util';
import { ethers } from 'ethers';
import { Queue, Worker } from 'bullmq';
// eslint-disable-next-line import/no-extraneous-dependencies
import { waitFor } from '@testing-library/react';
import { numMessagesConfig, numClientsConfig, connectionOptions, bobsAccount, accounts, loadTestQueueName, chainSpaceAndChannelJobName, jsonRpcProviderUrl, nodeRpcURL, loadTestShutdownQueueName, loadTestSignalCheckInterval, loadTestTimeout, } from './loadconfig.test_util';
...
describe('loadTestsScenario2', () => {
    test('create space, create channel, add #numClients users, send #numberOfMessages each, send signal to second jest', async () => {
        ...
        const bobClientWalletInfo = await createAndStartClient(bobsAccount, jsonRpcProviderUrl, nodeRpcURL);
        ...
        // create space
        const createTownReturnVal = await bobsRiverSDK.createSpaceWithDefaultChannel('load-test', '');
        const spaceStreamId = createTownReturnVal.spaceStreamId;
        // create channel
        const channelStreamId = await bobsRiverSDK.createChannel(spaceStreamId, 'load-tests', 'load-tests topic');
        ...
        const clientWalletInfos = await createAndStartClients(accounts.slice(0, numClientsConfig - 1), jsonRpcProviderUrl, nodeRpcURL);
        ...
        log(`${accounts.length} Clients join town and channel duration:${createClientsAndJoinTownAndChannelEnd - createClientsAndJoinTownAndChannelStart}`);
        ...
        for (let i = 0; i < numClientsConfig; i++) {
            for (let j = 0; j < numMessagesConfig; j++) {
                ...
                await client.sendMessage(channelId, `Message m_${j} from client_${i} with userId: ${client.userId}`);
                log('Sending Message Progress', (((i + 1) * 100) / numClientsConfig).toFixed(2), '%');
            }
        }
        const sendingMessageEndTime = Date.now();
        log('Send message done, time', sendingMessageEndTime - sendingMessageStartTime);
        log('Initiator duration from start to finshing sending message:', sendingMessageEndTime - loadTestStartTime);
        ...
        const _worker = new Worker(loadTestShutdownQueueName, 
        // eslint-disable-next-line
        async (job) => {
...
//# sourceMappingURL=initiator.test.js.map