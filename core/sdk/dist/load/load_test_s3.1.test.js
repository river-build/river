...
import { createAndStartClients, createClientSpaceAndChannel, getCurrentTime, multipleClientsJoinSpaceAndChannel, sendMessageAfterRandomDelay, wait, extractComponents, getUserStreamKey, getRandomElement, } from './load.test_util';
...
describe('loadTestsScenario: Main focus of this scenario is testing of DMs and GDMs. This scenario is supposed to be running for 10 minutes at 01:00 PST', () => {
    test('create three users, space, channel, sent messages to the channel at random time in a 10 minutes window for 3 hours.', async () => {
        ...
        const bobClientSpaceChannel = await createClientSpaceAndChannel(bobsAccount, jsonRpcProviderUrl, nodeRpcURL);
        ...
        const clientWalletInfos = await createAndStartClients(accounts.slice(0, 2), jsonRpcProviderUrl, nodeRpcURL);
        ...
        log('bobsStream.getMembers().membership.joinedUsers:', bobsStream.getMembers().membership.joinedUsers);
        ...
        const futureMessagesMap = new Map();
        function handleEventDecrypted(eventName, client) {
            ...
            client.on('eventDecrypted', async (streamId, contentKind, event) => {
                ...
                if (createdAtEpochMs > loadTestStartTime) {
                    ...
                    if (clearEvent.content.payload?.case === 'post' &&
                        ...
                        log(`${eventName}, client<${userStreamKey}> receives a message<${contentKind}>: ${body}`, messageStat);
                        ...
                        keyDebugLog.push(`${getCurrentTime()} ${eventName}, client<${userStreamKey}> receives a message<${contentKind}>: ${body}`);
                    }
                }
                else {
        ...
        for (let interval = 0; interval < totalDuration; interval += windowDuration) {
            ...
            setTimeout(() => startMessageSendingGDMWindow(windowIndex, allClients, channelId, futureMessagesMap, windowDuration), interval);
            log('Schedule sending DM message', { interval: interval, window: windowIndex });
            setTimeout(() => startMessageSendingDMWindow(windowIndex, allClients.slice(0, 2), // Only for bob and alice for now
            dmChannelReturnVal.streamId, futureMessagesMap, windowDuration), interval);
        ...
        function verifyMessagesReceivedByEndOfWindow(messagesSentPerUserMap, windowIndex) {
            log(`start verifyMessagesReceivedByEndOfWindow<${windowIndex}>`);
            keyDebugLog.push(`${getCurrentTime()} start verifyMessagesReceivedByEndOfWindow<${windowIndex}>`);
            messagesSentPerUserMap.forEach((sentMessagesSet, userStreamKey) => {
                if (sentMessagesSet.size === 0) {
                    ...
                    log(`Verification failure for ${userStreamKey} in window<${windowIndex}>`, sentMessagesSet);
                }
            });
        }
        ...
        await waitFor(() => {
            ...
            for (const [key, messageSet] of futureMessagesMap) {
                if (messageSet.size > 0) {
                    log(`Non-empty set found for key ${key}, with ${messageSet.size} messages, which means those message are not received.`, messageSet);
    ...
    const startMessageSendingDMWindow = (windowIndex, clients, channelId, messagesSentPerUserMap, windownDuration) => {
        const recipients = clients.map((client) => client.userId);
        for (let i = 0; i < clients.length; i++) {
            const client = getRandomElement(clients);
            if (client) {
                sendMessageAfterRandomDelay('dmChannelContent', client, recipients, channelId, windowIndex.toString(), messagesSentPerUserMap, windownDuration);
            }
        }
    };
    const startMessageSendingGDMWindow = (windowIndex, clients, channelId, messagesSentPerUserMap, windownDuration) => {
        const recipients = clients.map((client) => client.userId);
        for (let i = 0; i < clients.length; i++) {
            const client = clients[i];
            sendMessageAfterRandomDelay('gdmChannelContent', client, recipients, channelId, windowIndex.toString(), messagesSentPerUserMap, windownDuration);
        }
    };
});
//# sourceMappingURL=load_test_s3.1.test.js.map