...
import { RiverSDK, SpacesWithChannels, ChannelSpacePairs, ChannelTrackingInfo, pauseForXMiliseconds, getRandomInt, } from '../testSdk.test_util';
import { dlog } from '@river-build/dlog';
import { townsToCreate, channelsPerTownToCreate, followersNumber, jsonRpcProviderUrl, rpcClientURL, defaultChannelSamplingRate, defaultCoordinatorLeaveChannelsFlag, defaultRedisHost, defaultRedisPort, } from './stressconfig.test_util';
...
const loadOverUsers = new Set();
const usersInChannels = new Map();
const channelTrackingInfo = [];
...
const E2EDeliveryTimeMap = new Map();
const E2EDeliveryTimeHistogram = new Map([
    [500, 0],
    [1000, 0],
    [1500, 0],
    [2000, 0],
...
describe('Stress test', () => {
    test('stress test', async () => {
        ...
        const { spaceStreamId: coordinationSpaceId, defaultChannelStreamId: coordinationChannelId, } = await result.riverSDK.createSpaceAndChannel('main town', 'Controller Town', 'main channel');
                ...
                if (clearEvent.content?.payload?.case === 'post' &&
                    ...
                    if (streamId === coordinationChannelId) {
                        if (body === 'JOINED') {
                            ...
                            await result.riverSDK.sendTextMessage(coordinationChannelId, 'Follower with ID ' + event.creatorUserId + ' joined main channel');
        ...
        await result.riverSDK.sendTextMessage(coordinationChannelId, 'All followers joined main channel');
        //--------------------------------------------------------------------------------------------
        ...
        for (let i = 0; i < numTowns; i++) {
            const townCreationResult = await result.riverSDK.createSpaceWithDefaultChannel('Town ' + i, 'Channel 0 0');
            ...
            townsWithChannels.addChannelToSpace(townCreationResult.spaceStreamId, townCreationResult.defaultChannelStreamId);
            counter++;
            await result.riverSDK.sendTextMessage(coordinationChannelId, counter +
                ' channels of ' +
                totalNumberOfChannels +
                ' created' +
                townCreationResult.defaultChannelStreamId);
            channelWithTowns.addRecord(townCreationResult.defaultChannelStreamId, townCreationResult.spaceStreamId);
            for (let j = 0; j < numChannelsPerTown - 1; j++) {
                // -1 because we already created default channel
                const channelName = 'Channel ' + i + ' ' + j;
                const channelCreationResult = await result.riverSDK.createChannel(townCreationResult.spaceStreamId, channelName, '');
                ...
                townsWithChannels.addChannelToSpace(townCreationResult.spaceStreamId, channelCreationResult);
        ...
        await result.riverSDK.sendTextMessage(coordinationChannelId, 'WONDERLAND: ' + JSON.stringify(channelWithTowns));
        ...
        await result.riverSDK.sendTextMessage(coordinationChannelId, 'START LOAD: ' + JSON.stringify(channelTrackingInfo));
        ...
        do {
            ...
            const [nextCursor, keys] = await redisE2EMessageDeliveryTracking.scan(cursor, 'MATCH', '*', 'COUNT', 10);
                    ...
                    if (key.charAt(0) === 'S') {
                        ...
                        if (E2EDeliveryTimeMap.has(baseKey)) {
                            ...
                            if (receivingTime) {
                                ...
                                incrementE2EDeliveryTimeHistogramMapValue(diff, E2EDeliveryTimeHistogram);
...
function incrementE2EDeliveryTimeHistogramMapValue(sendTime, map) {
    let key;
...
async function getAllKeysAndValues(redisInstance) {
    const resultMap = new Map();
    let cursor = '0';
    do {
        // Use the SCAN command to iteratively get keys
        const reply = await redisInstance.scan(cursor, 'MATCH', '*', 'COUNT', 100);
        cursor = reply[0];
        const keys = reply[1];
        for (const key of keys) {
...
function extractTimestampAndConvertToDate(inputString) {
    const timestampString = inputString.substring(16);
    const date = new Date(timestampString);
    return date;
}
function findMinMaxDates(map) {
    if (map.size === 0) {
        return null;
    }
    let minDate = new Date(Number.MAX_SAFE_INTEGER);
    let maxDate = new Date(Number.MIN_SAFE_INTEGER);
