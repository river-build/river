...
import { MembershipOp, ChannelOp, ChannelMessage, ChannelMessage_Reaction, ChannelMessage_Redaction, FullyReadMarkers, Err, MemberPayload_Nft, } from '@river-build/proto';
...
import { GroupEncryptionCrypto, makeSessionKeys, } from '@river-build/encryption';
import { errorContains, getRpcErrorProperty } from './makeStreamRpcClient';
import EventEmitter from 'events';
import { isChannelStreamId, isDMChannelStreamId, isGDMChannelStreamId, isSpaceStreamId, makeDMStreamId, makeUniqueGDMChannelStreamId, makeUniqueMediaStreamId, makeUserDeviceKeyStreamId, makeUserSettingsStreamId, makeUserStreamId, makeUserInboxStreamId, userIdFromAddress, addressFromUserId, streamIdAsBytes, streamIdAsString, makeSpaceStreamId, STREAM_ID_STRING_LENGTH, } from './id';
import { makeEvent, unpackMiniblock, unpackStream, unpackStreamEx } from './sign';
import { StreamStateView } from './streamStateView';
import { make_UserDeviceKeyPayload_Inception, make_ChannelPayload_Inception, make_ChannelProperties, make_ChannelPayload_Message, make_MemberPayload_Membership2, make_SpacePayload_Inception, make_UserPayload_Inception, make_SpacePayload_Channel, make_UserSettingsPayload_FullyReadMarkers, make_UserSettingsPayload_UserBlock, make_UserSettingsPayload_Inception, make_MediaPayload_Inception, make_MediaPayload_Chunk, make_DMChannelPayload_Inception, make_DMChannelPayload_Message, make_GDMChannelPayload_Inception, make_GDMChannelPayload_Message, make_UserInboxPayload_Ack, make_UserInboxPayload_Inception, make_UserDeviceKeyPayload_EncryptionDevice, make_UserInboxPayload_GroupEncryptionSessions, make_GDMChannelPayload_ChannelProperties, make_UserPayload_UserMembershipAction, make_UserPayload_UserMembership, make_MemberPayload_DisplayName, make_MemberPayload_Username, getRefEventIdFromChannelMessage, make_ChannelPayload_Redaction, make_MemberPayload_EnsAddress, make_MemberPayload_Nft, } from './types';
...
export class Client extends EventEmitter {
    ...
    persistenceStore;
    constructor(signerContext, rpcClient, cryptoStore, entitlementsDelegate, persistenceStoreName, logNamespaceFilter, highPriorityStreamIds) {
        super();
        ...
        const shortId = shortenHexString(this.userId.startsWith('0x') ? this.userId.slice(2) : this.userId);
    ...
    async initUserDeviceKeyStream() {
        ...
        if (!(await userDeviceKeyStream.initializeFromPersistence())) {
            const response = (await this.getUserStream(this.userDeviceKeyStreamId)) ??
    ...
    async createUserDeviceKeyStream(userDeviceKeyStreamId) {
        const userDeviceKeyEvents = [
            await makeEvent(this.signerContext, make_UserDeviceKeyPayload_Inception({
    ...
    async createChannel(spaceId, channelName, channelTopic, inChannelId, streamSettings) {
    ...
    async createMediaStream(channelId, spaceId, chunkCount, streamSettings) {
    ...
    async sendFullyReadMarkers(channelId, fullyReadMarkers) {
        ...
        const fullyReadMarkersContent = new FullyReadMarkers({
    ...
    async initStream(streamId, allowGetStream = true) {
        try {
            ...
            if (stream) {
                ...
                if (!allowGetStream) {
                    ...
                    throw new Error(`Failed to initialize stream ${streamIdAsString(streamId)} from persistence`);
                }
                try {
                    const response = await this.rpcClient.getStream({
                        streamId: streamIdAsBytes(streamId),
    ...
    async sendChannelMessage(streamId, payload, opts = {}) {
        const stream = this.stream(streamId);
    ...
    async makeAndSendChannelMessageEvent(streamId, payload, localId) {
        const stream = this.stream(streamId);
        check(isDefined(stream), 'stream not found');
    ...
    async sendChannelMessage_Text(streamId, payload, opts = {}) {
        ...
        return this.sendChannelMessage(streamId, new ChannelMessage({
            payload: {
                case: 'post',
                value: {
                    ...options,
    ...
    async sendMediaPayload(streamId, data, chunkIndex, prevMiniblockHash) {
        const payload = make_MediaPayload_Chunk({
            data: data,
            chunkIndex: chunkIndex,
        });
        return this.makeEventWithHashAndAddToStream(streamId, payload, prevMiniblockHash, undefined, undefined);
    }
    ...
    async joinStream(streamId, opts) {
        ...
        if (opts?.skipWaitForUserStreamUpdate !== true) {
            if (!userStream.view.userContent.isJoined(streamIdStr)) {
                await userStream.waitFor('userStreamMembershipChanged', (streamId) => userStream.view.userContent.isJoined(streamId));
            }
        }
        return stream;
    }
    ...
    async downloadUserDeviceInfo(userIds) {
        ...
        const promises = userIds.map(async (userId) => {
            ...
            try {
                ...
                const userDevices = stream.userDeviceKeyContent.deviceKeys.slice(-deviceLookback);
    ...
    async makeEventAndAddToStream(streamId, payload, options = {}) {
        ...
        const { eventId } = await this.makeEventWithHashAndAddToStream(streamId, payload, prevHash, options.localId, options.cleartext);
        return eventId;
    }
    async makeEventWithHashAndAddToStream(streamId, payload, prevMiniblockHash, localId, cleartext, retryCount) {
        ...
        try {
            ...
            if (errorContains(err, Err.BAD_PREV_MINIBLOCK_HASH) && retryCount < 3) {
                ...
                this.logInfo('RETRYING event after BAD_PREV_MINIBLOCK_HASH response', retryCount, 'prevHash:', prevMiniblockHash, 'expectedHash:', expectedHash);
                check(isDefined(expectedHash), 'expected hash not found in error');
                return await this.makeEventWithHashAndAddToStream(streamId, payload, bin_fromHexString(expectedHash), isDefined(localId) ? eventId : undefined, cleartext, retryCount + 1);
            }
            else {
                if (localId) {
                    const stream = this.streams.get(streamId);
    ...
    async initCrypto() {
        ...
        this.decryptionExtensions = new ClientDecryptionExtensions(this, crypto, this.entitlementsDelegate, this.userId, this.userDeviceKey());
    }
    /**
    ...
    async uploadDeviceKeys() {
        ...
        return this.makeEventAndAddToStream(this.userDeviceKeyStreamId, make_UserDeviceKeyPayload_EncryptionDevice({
            ...this.userDeviceKey(),
    ...
    async encryptAndShareGroupSessions(inStreamId, sessions, toDevices) {
    ...
    encryptGroupEvent(event, streamId) {
        if (!this.cryptoBackend) {
            throw new Error('crypto backend not initialized');
        }
    ...
    async encryptWithDeviceKeys(payload, deviceKeys) {
        ...
        return this.cryptoBackend.encryptWithDeviceKeys(payload.toJsonString(), deviceKeys.filter((key) => key.deviceKey !== this.userDeviceKey().deviceKey));
    }
    // Used during testing
    userDeviceKey() {
        return {
