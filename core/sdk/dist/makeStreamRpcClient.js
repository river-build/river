...
const retryInterceptor = (retryParams) => {
    return (next) => async (req) => {
        ...
        while (true) {
            ...
            try {
                ...
                logError(req.method.name, 'ERROR RETRYING', attempt, 'of', retryParams.maxAttempts, 'retryDelay:', retryDelay, 'error:', e);
                await new Promise((resolve) => setTimeout(resolve, retryDelay));
            }
        }
    };
};
const loggingInterceptor = (transportId) => {
    // Histogram data structure
    const callHistogram = {};
    // Function to update histogram
    const updateHistogram = (methodName, suffix, error) => {
    ...
    setInterval(() => {
        if (Object.keys(callHistogram).length !== 0) {
            ...
            if (interval > 0) {
                logCallsHistogram('RPC stats for transportId=', transportId, 'interval=', interval, 'total=', total, 'error=', error, 'intervalMs=', histogramIntervalMs, '\n', sortObjectKey(callHistogram));
                for (const key in callHistogram) {
    ...
    return (next) => async (req) => {
        ...
        if (req.stream) {
            ...
            const streamIdBytes = req.message['streamId'];
    ...
    async function* logEachRequest(name, id, stream) {
        try {
    ...
    async function* logEachResponse(name, id, stream) {
        try {
            for await (const m of stream) {
                try {
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
...
export function errorContains(err, error) {
...
export function getRpcErrorProperty(err, prop) {
    if (err !== null && typeof err === 'object' && 'message' in err) {
        const expected = `${prop} = `;
        const parts = err.message.split(expected);
        if (parts.length === 2) {
...
function getRetryDelay(error, attempts, retryParams) {
    ...
    const retryDelay = Math.min(retryParams.maxRetryDelay, retryParams.initialRetryDelay * Math.pow(2, attempts));
...
export function makeStreamRpcClient(dest, retryParams = { maxAttempts: 3, initialRetryDelay: 2000, maxRetryDelay: 6000 }, refreshNodeUrl) {
