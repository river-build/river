 ...
 */
import { getDynamicPricingModule, makeDonePromise, makeTestClient, makeUserContextFromWallet, waitFor, } from './util.test';
...
describe('spaceWithEntitlements', () => {
    ...
    test('ownerCanBanOtherUsers', async () => {
        ...
        const channelReturnVal = await bob.createChannel(spaceId, 'general', channelProperties, channelId);
        ...
        await waitFor(() => {
            expect(aliceUserStreamView.userContent.isMember(spaceId, MembershipOp.SO_JOIN)).toBe(true);
            expect(aliceUserStreamView.userContent.isMember(channelId, MembershipOp.SO_JOIN)).toBe(true);
        });
        // Alice cannot kick Bob
        await expect(alice.removeUser(spaceId, bob.userId)).rejects.toThrow(expect.objectContaining({
    ...
    test('oneNftGateJoinPass', async () => {
        ...
        const aliceMintTx = publicMint('TestNFT1', alicesWallet.address);
        ...
        log('transaction start Alice joining space');
        const { issued, tokenId } = await aliceSpaceDapp.joinSpace(spaceId, alicesWallet.address, aliceProvider.wallet);
        expect(issued).toBe(true);
