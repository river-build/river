...
describe('dmsTests', () => {
    ...
    test('clientsAreJoinedAutomaticallyAndCanLeaveDM', async () => {
        ...
        await waitFor(() => {
            expect(stream.view.getMembers().membership.joinedUsers).toEqual(new Set([bobsClient.userId]));
        });
    });
    ...
    test('otherUsersCantJoinDM', async () => {
        ...
        const { streamId } = await bobsClient.createDMChannel(alicesClient.userId);
        await expect(charliesClient.joinStream(streamId, { skipWaitForMiniblockConfirmation: true })).toReject();
    });
    ...
    test('usersReceiveKeys', async () => {
        ...
        await expect(bobsClient.waitForStream(streamId)).toResolve();
        await expect(alicesClient.waitForStream(streamId)).toResolve();
        const aliceEventDecryptedPromise = createEventDecryptedPromise(alicesClient, 'hello this is bob');
        const bobEventDecryptedPromise = createEventDecryptedPromise(bobsClient, 'hello bob, this is alice');
        ...
        await expect(Promise.all([aliceEventDecryptedPromise, bobEventDecryptedPromise])).toResolve();
    });
    ...
    test('clientCannotCreateSingleParticipantDMForOtherUser', async () => {
        ...
        const inceptionEvent = await makeEvent(alicesClient.signerContext, make_DMChannelPayload_Inception({
            streamId: channelId,
