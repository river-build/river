...
describe('clientFlushes', () => {
    ...
    test.skip('bobTalksToHimself-flush', async () => {
        ...
        const onChannelNewMessage = (channelId, streamKind, event) => {
            ...
            try {
                ...
                if (clearEvent.content.payload?.case === 'post' &&
                    ...
                    expect(clearEvent.content.payload?.value?.content.value?.body).toContain('Hello, world!');
        ...
        await sendFlush(bobsClient.rpcClient);
        await expect(bobsClient.createChannel(bobsSpaceId, bobsChannelName, bobsChannelTopic, makeUniqueChannelStreamId(bobsSpaceId))).toResolve();
        await sendFlush(bobsClient.rpcClient);
    ...
    const bobCanReconnect = async (log) => {
            ...
            void (async () => {
                try {
                    if (streamKind === 'channelContent') {
                        ...
                        log('channel content');
                        log(channel.view);
                        const messages = channel.view.timeline.filter((x) => x.decryptedContent?.kind === 'channelMessage');
...
//# sourceMappingURL=clientFlushes.test.js.map