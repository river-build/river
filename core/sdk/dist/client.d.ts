...
import { ChannelMessage_Post_Mention, ChannelMessage, ChannelMessage_Post, ChannelMessage_Post_Content_Text, ChannelMessage_Post_Content_Image, ChannelMessage_Post_Content_GM, ChannelMessage_Reaction, ChannelMessage_Redaction, StreamEvent, EncryptedData, StreamSettings, FullyReadMarker, Envelope, ChannelMessage_Post_Attachment } from '@river-build/proto';
...
export declare class Client extends Client_base implements IGroupEncryptionClient {
    ...
    updateChannel(spaceId: string | Uint8Array, channelId: string | Uint8Array, unused1: string, unused2: string): Promise<string>;
    updateGDMChannelProperties(streamId: string, channelName: string, channelTopic: string): Promise<string>;
    sendFullyReadMarkers(channelId: string | Uint8Array, fullyReadMarkers: Record<string, FullyReadMarker>): Promise<string>;
    updateUserBlock(userId: string, isBlocked: boolean): Promise<string>;
    ...
    sendMessage(streamId: string, body: string, mentions?: ChannelMessage_Post_Mention[], attachments?: ChannelMessage_Post_Attachment[]): Promise<string>;
    ...
    sendChannelMessage_Redaction(streamId: string, payload: PlainMessage<ChannelMessage_Redaction>): Promise<string>;
    sendChannelMessage_Edit(streamId: string, refEventId: string, newPost: PlainMessage<ChannelMessage_Post>): Promise<string>;
    sendChannelMessage_Edit_Text(streamId: string, refEventId: string, payload: Omit<PlainMessage<ChannelMessage_Post>, 'content'> & {
        content: PlainMessage<ChannelMessage_Post_Content_Text>;
    }): Promise<string>;
    redactMessage(streamId: string, eventId: string): Promise<string>;
    retrySendMessage(streamId: string, localId: string): Promise<void>;
    inviteUser(streamId: string | Uint8Array, userId: string): Promise<string>;
    joinUser(streamId: string | Uint8Array, userId: string): Promise<string>;
    joinStream(streamId: string | Uint8Array, opts?: {
        skipWaitForMiniblockConfirmation?: boolean;
        skipWaitForUserStreamUpdate?: boolean;
    }): Promise<Stream>;
    leaveStream(streamId: string | Uint8Array): Promise<string>;
    removeUser(streamId: string | Uint8Array, userId: string): Promise<string>;
    ...
    makeEventWithHashAndAddToStream(streamId: string | Uint8Array, payload: PlainMessage<StreamEvent>['payload'], prevMiniblockHash: Uint8Array, localId?: string, cleartext?: string, retryCount?: number): Promise<{
        prevMiniblockHash: Uint8Array;
        eventId: string;
    }>;
    ...
    uploadDeviceKeys(): Promise<string>;
