...
describe('mediaTests', () => {
    ...
    async function bobCreateMediaStream(chunkCount) {
    ...
    async function bobSendMediaPayloads(streamId, chunks, prevMiniblockHash) {
    ...
    test('chunkIndexNeedsToBeWithinBounds', async () => {
        ...
        const chunk = new Uint8Array(100);
        await expect(bobsClient.sendMediaPayload(result.streamId, chunk, -1, result.prevMiniblockHash)).toReject();
        await expect(bobsClient.sendMediaPayload(result.streamId, chunk, 11, result.prevMiniblockHash)).toReject();
    });
    test('chunkSizeCanBeAtLimit', async () => {
        const result = await bobCreateMediaStream(10);
        const chunk = new Uint8Array(500000);
        await expect(bobsClient.sendMediaPayload(result.streamId, chunk, 0, result.prevMiniblockHash)).toResolve();
    });
    test('chunkSizeNeedsToBeWithinLimit', async () => {
        const result = await bobCreateMediaStream(10);
        const chunk = new Uint8Array(500001);
        await expect(bobsClient.sendMediaPayload(result.streamId, chunk, 0, result.prevMiniblockHash)).toReject();
    });
    ...
    test('clientCanOnlyPostToTheirOwnMediaStream', async () => {
        ...
        await expect(alicesClient.sendMediaPayload(result.streamId, chunk, 5, result.prevMiniblockHash)).toReject();
    ...
    test('channelNeedsToExistBeforeCreatingMediaStream', async () => {
        ...
        await expect(bobsClient.createMediaStream(nonExistentChannelId, nonExistentSpaceId, 10)).toReject();
    });
    ...
    test.skip('mediaStreamGetStreamEx', async () => {
        ...
        await bobsClient.rpcClient.info(new InfoRequest({
