...
import { baseChainConfig, testRunTimeMs, connectionOptions, loginWaitTime, followerKey, jsonRpcProviderUrl, fromFollowerQueueName, fromLeaderQueueName, riverNodeRpcUrl, } from './30MinutesSyntheticConfig.test_util';
...
describe('mirrorMessages', () => {
    test('mirrorMessages', async () => {
        ...
        const _followerWorker = new Worker(fromLeaderQueueName, 
        // eslint-disable-next-line
        async (message) => {
            ...
            if (commandData.commandType === 'messageSent') {
                ...
                if (clearEvent.content?.payload?.case === 'post' &&
                    ...
                    clearEvent.content?.payload?.value?.content.value?.body ===
                        commandData.command) {
                    await client.sendMessage(commChannelId, 'Mirror from Bot 2: ' + commandData.command);
                    log('Reply message sent with text: ', 'Mirror from Bot 2: ' + commandData.command);
                    replyWasSent.done();
                }
            }
            if (commandData.commandType === 'joinSpace') {
                const spaceDapp = createSpaceDapp(walletWithProvider.provider, baseChainConfig);
                const spaceAndChannelIds = commandData.command;
                const hasMembership = await spaceDapp.hasSpaceMembership(spaceAndChannelIds.townId, walletWithProvider.address);
                if (!hasMembership) {
                    // mint membership
                    const { issued } = await spaceDapp.joinSpace(spaceAndChannelIds.townId, walletWithProvider.address, walletWithProvider);
...
//# sourceMappingURL=30MinutesSyntheticFollower.test.js.map