...
import { baseChainConfig, testRunTimeMs, connectionOptions, loginWaitTime, leaderKey, jsonRpcProviderUrl, fromFollowerQueueName, fromLeaderQueueName, envName, defaultEnvironmentName, testSpamChannelName, riverNodeRpcUrl, followerId, followerKey, followerUserName, leaderId, leaderUserName, replySentTime, } from './30MinutesSyntheticConfig.test_util';
...
describe('mirrorMessages', () => {
    test('mirrorMessages', async () => {
        ...
        if (envName !== defaultEnvironmentName) {
            log('Creating town');
            const result = await riverSDK.createSpaceWithDefaultChannel('test town', 'town metadata', testSpamChannelName);
            log('result', result);
            ...
            const redirectLocation = await checkRedirectLocation('https://latest-dev-town.towns.com/');
            ...
            if (!testChannelId) {
                testChannelId = await riverSDK.createChannel(testTownId, testSpamChannelName, testSpamChannelName);
            }
        }
        await healthcheckQueueLeader.add(fromLeaderQueueName, {
        ...
        const messageText = crypto.getRandomValues(new Uint8Array(16)).toString() + ' ' + isoDateString;
...
async function checkRedirectLocation(url) {
    try {
...
//# sourceMappingURL=30MinutesSyntheticLead.test.js.map