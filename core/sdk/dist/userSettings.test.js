...
describe('userSettingsTests', () => {
    ...
    test('clientCanBlockUser', async () => {
        ...
        await waitFor(() => {
            expect(bobsClient.stream(bobsClient.userSettingsStreamId)?.view?.userSettingsContent
                ?.userBlocks[alicesClient.userId]?.blocks.length).toBe(1);
            expect(bobsClient
                .stream(bobsClient.userSettingsStreamId)
                ?.view?.userSettingsContent?.isUserBlocked(alicesClient.userId)).toBe(true);
        });
        // bob unblocks alice, there will be two blocks
        await bobsClient.updateUserBlock(alicesClient.userId, false);
        await waitFor(() => {
            expect(bobsClient.stream(bobsClient.userSettingsStreamId)?.view?.userSettingsContent
                ?.userBlocks[alicesClient.userId]?.blocks.length).toBe(2);
            expect(bobsClient
                .stream(bobsClient.userSettingsStreamId)
                ?.view?.userSettingsContent?.isUserBlocked(alicesClient.userId)).toBe(false);
        });
        // bob blocks alice again, there will be three blocks
        await bobsClient.updateUserBlock(alicesClient.userId, true);
        await waitFor(() => {
            expect(bobsClient.stream(bobsClient.userSettingsStreamId)?.view?.userSettingsContent
                ?.userBlocks[alicesClient.userId]?.blocks.length).toBe(3);
            expect(bobsClient
                .stream(bobsClient.userSettingsStreamId)
                ?.view?.userSettingsContent?.isUserBlocked(alicesClient.userId)).toBe(true);
        });
    ...
    test('DMMessagesAreBlockedDuringBlockedPeriod', async () => {
        ...
        await waitFor(() => {
            expect(bobsClient.stream(streamId)?.view?.timeline?.filter((m) => {
                return (m.creatorUserId === alicesClient.userId &&
                    bobsClient
                        .stream(bobsClient.userSettingsStreamId)
                        ?.view?.userSettingsContent?.isUserBlockedAt(alicesClient.userId, m.eventNum));
            }).length).toBe(3);
        });
...
//# sourceMappingURL=userSettings.test.js.map