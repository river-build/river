import { Err, SyncOp } from '@river-build/proto';
import { dlog, dlogError, shortenHexString } from '@river-build/dlog';
...
export class SyncedStreams {
    ...
    constructor(userId, rpcClient, clientEmitter) {
        this.userId = userId;
    ...
    async addStreamToSync(syncCookie) {
        ...
        if (this.syncState === SyncState.Syncing) {
            ...
            this.log('addStreamToSync: not in "syncing" state; let main sync loop handle this with its streams map', { streamId: syncCookie.streamId, syncState: this.syncState });
        }
    }
                    ...
                    while (this.syncState === SyncState.Starting ||
                        ...
                        try {
                            ...
                            while (this.syncState === SyncState.Syncing ||
                                ...
                                this.log('got syncStreams response', 'syncOp', value.syncOp, 'syncId', value.syncId);
                                ...
                                switch (value.syncOp) {
                                    ...
                                    case SyncOp.SYNC_PONG:
                                        ...
                                        break;
                                    default:
                                        this.log(`unknown syncOp { syncId: ${this.syncId}, syncOp: ${value.syncOp} }`);
                                        break;
                                }
                            }
                        }
                ...
                try {
                    ...
                    */
                    ...
                    if (stream === undefined) {
                        ...
                        await stream.appendEvents(streamAndCookie.events, streamAndCookie.nextSyncCookie, undefined);
                    }
                }
                catch (err) {
                    const e = err;
