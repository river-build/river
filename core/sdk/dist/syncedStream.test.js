...
describe('syncedStream', () => {
    test('clientRefreshesStreamOnBadSyncCookie', async () => {
        ...
        expect(bobStreamFresh.view.timeline.map((e) => e.remoteEvent)).toEqual(bobStreamCached.view.timeline.map((e) => e.remoteEvent));
        expect(aliceStream.view.timeline.length).toBeGreaterThan(bobStreamFresh.view.timeline.length);
        // wait for new stream to trigger bad_sync_cookie and get a fresh view sent back
        await waitFor(() => bobStreamFresh.view.miniblockInfo.max > bobStreamCached.view.miniblockInfo.max);
        ...
        await waitFor(() => aliceStream.view.miniblockInfo.max === bobStreamFresh.view.miniblockInfo.max);
