...
// NewPrettyJSONHandler creates a PrettyJSONHandler that writes to w,
// using the given options.
// If opts is nil, the default options are used.
func NewPrettyJSONHandler(w io.Writer, opts *PrettyHandlerOptions) *PrettyJSONHandler {
	if opts == nil {
...
func appendJSONMarshal(buf *buffer.Buffer, v any) error {
	...
	bs := bb.Bytes()
	_, _ = buf.Write(bs[:len(bs)-1]) // remove final newline
	return nil
}
