...
func (up *UserPreferences) Clone() *UserPreferences {
	...
	for _, webPush := range up.Subscriptions.WebPush {
...
func DecodeUserPreferenceFromMsg(msg *SetSettingsRequest) (*UserPreferences, error) {
	userID := common.BytesToAddress(msg.GetUserId())
	if userID == (common.Address{}) {
		return nil, RiverError(Err_INVALID_ARGUMENT, "Missing user id")
	}
