// todo: remove this file once the updated SDK is published
...
export async function decryptDerivedAESGCM(
	...
	if (encryptedData.algorithm !== AES_GCM_DERIVED_ALGORITHM) {
		throw new Error(`${encryptedData.algorithm} algorithm not implemented`);
	}
...
/*
export async function decryptAESGCM(data: Uint8Array, iv: Uint8Array, secretKey: Uint8Array): Promise<ArrayBuffer> {
	// Import the key into the SubtleCrypto API
	const aesCryptoKey = await crypto.subtle.importKey('raw', secretKey, { name: 'AES-GCM' }, false, ['decrypt']);
